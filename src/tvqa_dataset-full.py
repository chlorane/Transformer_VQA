...
#from transformers import (WEIGHTS_NAME, AdamW, BertConfig, BertForMultipleChoice, BertTokenizer, RobertaConfig, RobertaForMultipleChoice, RobertaTokenizer, XLNetConfig, XLNetForMultipleChoice, XLNetTokenizer, get_linear_schedule_with_warmup)
import nltk
...
def tfidf(query):
    ...
    Tfidf = transformer.fit_transform(loaded_vec.transform(query))
    Tfidf=Tfidf.toarray()
    return Tfidf

#def tfidf(sentence,query):
#    corpus = [sentence,query]
...
def cosinesimilarity(tfidf_a,tfidf_b):
    cx=lambda a, b : round(np.inner(a, b)/(LA.norm(a)*LA.norm(b)), 3)
    cosine=cx(tfidf_a, tfidf_b)
    return cosine
...
#        Endings=example.endings[0]+example.endings[1]+example.endings[2]+example.endings[3]+example.endings[4]
        context_tokens=example.context_sentence #question
#        CT=context_tokens.lower()+example.endings[0]+example.endings[1]+example.endings[2]+example.endings[3]+example.endings[4]
#        t_context=nltk.word_tokenize(CT)
#        context_tokens_nostwd=' '.join([word for word in t_context if word not in stopword])
#        ED=Endings.lower()
#        t_ED=nltk.word_tokenize(ED)
#        ED_nostwd=' '.join([word for word in t_ED if word not in stopword])
#        subtitle=subtitle.replace("<eos>","")
#        t_subtitle=nltk.word_tokenize(subtitle)
#        thresh=80
#        step=10
#        if(len(t_subtitle)>thresh):
#            w_len=math.ceil((len(t_subtitle)-thresh)/step)+1
#        else:
#            w_len=1
#        subtitles=[0]*w_len
#        cossimis=[0]*w_len
#        for i in range(w_len):
#            subtitles[i]=t_subtitle[i*step:min(len(t_subtitle),i*step+thresh-1)]
#            subtitles[i]=" ".join(subtitles[i])
#            subtitles[i]=subtitles[i].lower()
#            Tfidf=tfidf(subtitles[i],context_tokens_nostwd)
#            cossimis[i]=cosinesimilarity(Tfidf[0],Tfidf[1])
#        scossimi=sorted(range(len(cossimis)), key=lambda k: cossimis[k], reverse=True)
#        subtitle_tokens=subtitles[scossimi[0]]
#        if(subtitle_tokens==0):
#            print("subtitle_tokens==0")
...
#        self.glove_embedding_path = opt.glove_path
    ...
    def __len__(self):
#        print("self.cur_data_dict="+str(self.cur_data_dict))
    ...
    def __getitem__(self, index):
        ...
        if self.with_ts:
            R_sub=self.cur_data_dict[index]["located_sub_text"]
        else:
...
#            items.append(self.numericalize(raw_words))
