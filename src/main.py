...
#    ALL_MODELS = sum((tuple(conf.pretrained_config_archive_map.keys()) for conf in (BertConfig, XLNetConfig, RobertaConfig, AlbertConfig)), ())
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader)):
        model_inputs, targets, _ = preprocess_inputs(batch, opt.max_sub_l, opt.max_vcpt_l, tmoname, transformer_tokenizer, opt.max_seq_length, opt.sep_char, device=opt.device)
